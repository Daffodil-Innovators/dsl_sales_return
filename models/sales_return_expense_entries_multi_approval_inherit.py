...
class MultiApprovalTypeInherit(models.Model):
    ...
    sales_return_expense_entries_id = fields.Many2one('sales.return.expense.entries', string='Sales Return')
...
class MultiApproval(models.Model):
    ...
    def action_approve(self):
        ...
        for rec in recs:
            ...
            model = self.env['ir.model'].sudo().search([('model', '=', 'sales.return.expense.entries')], order='id desc', limit=1)
            if model:
                if rec.state == 'Draft':
                    sales_return = self.env['sales.return.expense.entries'].sudo().search([('approve_id', '=', rec.id)], order='id desc', limit=1)
